...
def _phase(registry: CollectorRegistry, phase: str) -> float | None:
    return registry.get_sample_value("tkati_phase_seconds_total", {"phase": phase})


def _value(registry: CollectorRegistry, metric: str) -> float | None:
    return registry.get_sample_value(metric)
...
def test_every_declared_phase_is_exported_before_it_fires() -> None:
    ...
    registry = _registry(LoopStats(phases=PHASES))
...
def test_counters_carry_the_loop_stats() -> None:
    ...
    assert _value(registry, "tkati_rows_in_total") == 10
    assert _value(registry, "tkati_rows_out_total") == 8
    assert _value(registry, "tkati_iterations_total") == 3
    assert _value(registry, "tkati_starved_iterations_total") == 1
...
def test_counters_do_not_go_backwards_across_a_report() -> None:
    ...
    stats.rows_in = 10
    before = (_phase(registry, "work"), _value(registry, "tkati_rows_in_total"))

    stats.report()

    after = (_phase(registry, "work"), _value(registry, "tkati_rows_in_total"))
    assert after == before
...
def test_wall_clock_bounds_the_sum_of_phases() -> None:
    ...
    wall = _value(registry, "tkati_wall_seconds_total")
...
def test_disabled_settings_start_nothing() -> None:
    ...
    server = start_metrics_server(
        MetricsSettings(enabled=False), LoopStats(phases=()), registry
    )
...
def test_server_serves_the_stats_over_http() -> None:
    ...
    assert "tkati_rows_in_total 42.0" in body
    assert 'tkati_phase_seconds_total{phase="consumer/poll"} 0.0' in body
